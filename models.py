...
class Exam(Base):
    ...
    status = Column(String(20), default='进行中')  # 考试状态:进行中、已完成、已过期
...
class ExamRecord(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    student_id = Column(String(20), ForeignKey('users.student_id'))
    exam_id = Column(String(20), ForeignKey('exams.exam_id'))
    question_id = Column(String(20))
    student_answer = Column(String(500))  # 存储学生的答案,可能是字符串、列表或布尔值的JSON字符串
    is_correct = Column(Boolean)
...
class AIChatRecord(Base):
    ...
    is_irrelevant = Column(Boolean, default=False)  # 是否是无关问题
